...
mutable struct Document <: ServerExtension
    type::Vector{Symbol}
    f::Function
    servables::Dict
    function Document()
        f(c::Connection) = begin
            ...
            var bodyHtml = document.getElementsByTagName('body')[0].innerHTML;
              ...
              xhttp.open("GET", "/action_page.php?tlbody=" + bodyHtml, true);
            xhttp.send();
            }
            """)
            servables = html_tservable(getarg(c, :tlbody))
        end
        new([:connection, :func], f, Dict())
    end
end

function write!(c::Connection, d::Document)
    for servable in d.servables
        write!(c, servable)
    end
end

getindex(d::Document, sname::String, prop::String) = d.servables[sname][prop]

function setindex(d::Document, sname::String, prop::Pair)
    d.servables[sname][prop[1]] = prop[2]
end

function html_tservable(s::String)
    """<div align = "center"><a>hello <p>great</p></a></div>"""
    open_tags = findall("<", s)
    close_tags = findall(">", s)
    open_close = [open_tags[i]:close_tags[i] for i in length(open_tags)]
    servables = []
    for n in 1:length(open_close)
        ...
        tagname_r = tagrange[1] + 1:findnext(" ", s, tagrange[1])
        tagname = s[tagname_r]
        nextagr = open_close[n + 1]
        unsplit_props = s[tagname_r[1] + 1:tagrange[2] - 1]
        props = parse(Dict, replace(unsplit_props, " " => ","))
        cname = props[:name]
        cname[:children] = Vector{Servable}()
        c = Component(cname, tagname, props)
    ...
    return(Dict([s.name => s for s in servables]))
end
